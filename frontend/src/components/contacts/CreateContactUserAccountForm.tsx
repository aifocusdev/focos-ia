...
import type { CreateContactUserAccountRequest } from '../../types/contact-user-accounts.types'

interface CreateContactUserAccountFormProps {
  contactId: string
  isLoading: boolean
  onSubmit: (data: CreateContactUserAccountRequest) => Promise<void>
  onCancel: () => void
}
...
const CreateContactUserAccountForm: React.FC<CreateContactUserAccountFormProps> = ({
  contactId,
  isLoading,
  onSubmit,
  onCancel
  ...
  const handleFormSubmit = async (data: Record<string, unknown>) => {
    try {
      ...
      const createData: CreateContactUserAccountRequest = {
        contact_id: parseInt(contactId),
        username_final: data.username_final as string,
        password_final: data.password_final as string,
        server_id: Number(data.server_id),
        id_line_server: data.id_line_server ? parseInt(data.id_line_server as string) : undefined,
        date_exp: data.date_exp ? new Date(data.date_exp as string).toISOString() : undefined,
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      ...
      <Input
        label="ID Line Server (opcional)"
        type="text"
        {...register('id_line_server')}
        error={errors.id_line_server?.message}
        disabled={isLoading}
        placeholder="Digite o ID da linha do servidor"
      />
...
export default CreateContactUserAccountForm