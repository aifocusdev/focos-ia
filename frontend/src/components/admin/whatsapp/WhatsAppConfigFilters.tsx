...
const WhatsAppConfigFilters: React.FC = () => {
  const { setFilters, fetchConfigs, loading } = useWhatsAppStore()
  const [searchValue, setSearchValue] = useState('')

  const handleSearch = async () => {
    const filters = searchValue.trim() ? { phone_number_id: searchValue.trim() } : {}
    setFilters(filters)
    ...
    <div className="bg-gray-800 rounded-2xl shadow-2xl p-6 mb-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Buscar por Phone Number ID..."
            value={searchValue}
...
export default WhatsAppConfigFilters